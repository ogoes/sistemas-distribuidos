...
TCPSocket::~TCPSocket () {}
TCPSocket::TCPSocket (io_context & context, tcp type, unsigned int PORT)
    : acceptor (context, tcp::endpoint (type, PORT)), socket (context),
      resolver (context) {}
tcp::socket TCPSocket::acceptConnection () { return this->acceptor.accept (); }
...
void TCPSocket::sendMessage (tcp::socket & destination_socket,
                             const std::string message) {
  boost::asio::write (destination_socket,
                      boost::asio::buffer (message.c_str (), message.size ()));
}
void TCPSocket::close () { this->socket.close (); }

tcp::socket ServerSocket::acceptConnection () {
  if (socket_p == protocols::UDP)
    throw "ERRO: UDP protocol doesn't need to accept connections";
...
void ServerSocket::sendMessage (tcp::socket & destination_socket,
                                const std::string message) {
