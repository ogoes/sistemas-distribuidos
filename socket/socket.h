...
/**
 * Classe responsável por criar um servidor que responde por protocolos
 * (TCP/UDP)
 */
class ServerSocket {
private: // attributes
  ...
  /**
   * Métodos responsáveis por criarem o objeto com base no protocolo de
   * comunicação (TCP/UDP) que o servidor usará
   *
   * @param
...
public: // métodos
   ...
   * BLOQUEANTE: A execução do programa é "pausada" até que uma conexão seja
   * estabelecida
   *
   * @return
   * soquete TCP (Não é necessário para conexões UDP) que identifica o cliente
   * que acabou de conectar-se ao servidor
   *
   */
  tcp::socket acceptConnection ();
   ...
   * BLOQUEANTE: A execução do programa é "pausada" até que a mensagem seja
   * recebida
   *
   * @param
  ...
  /**
   * Método responsável por enviar uma messagem para o processo conectado com o
   * servidor
   *
   * @param
 ...
 * Se trata de uma abordagem bloqueante, assim o programa "espera" o recebimento
 * da mensagem Cabe ao programador tratar o multiprocessamento/assincronismo se
 * necessário
 */
class ClientSocket {
private: // atributos
...
public: // métodos
  /**
   * Métodos responsáveis por definir qual protocolo de comunicação (TCP/UDP) a
   * instância usará.
   *
   * @param
   * const char * HOST: endereço IP (IPv4/IPv6) host que está executando o
   * servidor const char * PORT: porta aberta pelo servidor
   *
   * @return
  ...
  /**
   * Método responsável por finalizar a comunicação entre o soquete cliente e o
   * servidor
   *
   * @param
   *
